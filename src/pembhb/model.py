...
class ReducedOrderModel:
    ...
    def __init__(self, batch_size=1000, tolerance=1e-3, device="cpu",
                 filename=None, debugging=False, domain='fd'):
    ...
    def _train_single_domain(self, dom, train_dataloader, convergence_on):
        ...
        n_dim = sample.shape[1]  # n_freq for fd, n_time for td
        setattr(self, f'n_channels_{dom}', n_channels)
        setattr(self, f'n_dim_{dom}', n_dim)

        # Compute global scale & mean for this domain
        scale, mean = self._compute_global_scale(train_dataloader, wave_key, n_channels * n_dim)
    ...
    def _max_residual_index_dom(self, dom, train_dataloader, picked_set, diagnostics, prefetch_batches: int = 1):
        ...
        while True:
            ...
            norms_residual_data = ((wave_batch - data_proj).abs() ** 2).sum(dim=1)
    ...
    def _compute_global_scale(self, dataloader, wave_key='wave_fd', flat_dim=None):
        ...
        peek = next(iter(dataloader))[wave_key]
...
class ROMWrapper(nn.Module): 
    ...
    def get_n_features(self):
        ...
        n = 0
        for dom in self.rom._active_domains:
    ...
    def forward(self, d_f, d_t): 
        ...
        :return: tuple of (compressed_features, passthrough) where compressed_features
                 is real-valued with shape (B, n_features).
        """
        parts = []
        passthrough = None

        if hasattr(self.rom, 'basis_fd') and self.rom.basis_fd is not None:
            ...
            passthrough = d_f  # pass through uncompressed

        if hasattr(self.rom, 'basis_td') and self.rom.basis_td is not None:
            compressed_td = self.rom.compress(d_t, dom='td')
            parts.append(compressed_td)  # already real
        else:
            if passthrough is None:
                passthrough = d_t  # pass through uncompressed

        if parts:
            compressed = torch.cat(parts, dim=1)
        else:
            compressed = None
