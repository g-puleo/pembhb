...
\title{Simulation-Based inference for massive black hole binary from mock LISA data }
\author{Gianmarco Puleo, R. Trotta, E. Barausse}
...
\begin{column}{0.16\paperwidth}
  \centering
  \includegraphics[height=8.7cm]{sissa_logo.pdf}
\end{column}
\begin{column}{0.66\paperwidth}
  \centering
  {\usebeamerfont{title}\usebeamercolor[fg]{title}\linespread{1.15}\selectfont\inserttitle}\\[0.6cm]
...
\begin{column}{0.47\paperwidth}
  ...
  \begin{block}{TMNRE method: architecture}
    \centering
    \includegraphics[width=0.95\linewidth]{architecture_chmlp_tmnre_simple_crop.pdf}
...
\begin{column}{0.47\paperwidth}
  ...
  \begin{block}{Multimodal truncation (mask example)}
    \noindent
    ...
    \vspace{3mm}
    \centering
    \includegraphics[width=0.95\linewidth]{cherry_posterior_sky_round25.pdf}
  \end{block}
  ...
  \begin{block}{\textcolor{highlight}{Final posterior vs.\ MCMC}}
    \cardbox[highlight]{%
      \begin{minipage}{\dimexpr\linewidth-2\cardpad\relax}
        ...
        \begin{minipage}[c]{0.48\linewidth}
          \centering
          \includegraphics[width=\linewidth]{final_posterior_zoom.pdf}
        \end{minipage}%
        \hfill
        \begin{minipage}[c]{0.48\linewidth}
          We train our model to produce posteriors for the 11 source parameters. 
          The final-round posterior agrees with that of an MCMC sampler initialised close to the true value -- our method recovers the likelihood-based result without ever evaluating the likelihood.
        \end{minipage}
      \end{minipage}%
    }
  \par
